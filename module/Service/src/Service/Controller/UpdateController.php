...
class UpdateController extends AbstractActionController
{
    ...
    {
        //endpoint for updating shipping info to order        
        ...
        try {
            ...
            $inputFilter = $factory->createInputFilter(array(
                ...
                array('name' => 'shippingId',
                      ...
                      'validators' => array(array('name' => 'Zend\Validator\NotEmpty',
                                                  'break_chain_on_failure' => true,
                                                  'options' => array('messages' => array(\Zend\Validator\NotEmpty::IS_EMPTY => "shippingId cannot be empty"))
                                                 ),
                                            array('name' => 'Zend\I18n\Validator\Alnum',
                                                  'break_chain_on_failure' => true,
                                                  'options' => array('allowWhiteSpace' => false,
                                                                     'messages' => array(\Zend\I18n\Validator\Alnum::INVALID      => "Invalid data type given for shippingId",
                                                                                         \Zend\I18n\Validator\Alnum::NOT_ALNUM    => "shippingId must only be alphanumeric",
                                                                                         \Zend\I18n\Validator\Alnum::STRING_EMPTY => "shippingId cannot be empty",
                                                                                        ),
                                                                    ),
                                                 ),
                                           ),
                ),
                array('name' => 'shippingStatus',
                      'required' => true,
                      //note: 'allow_empty' and 'continue_if_empty' options are deprecated since zf 2.4.8, to disallow this field to accept empty value, set these options to true and add a Zend\Validator\NotEmpty instance to the validator chain instead
                      'allow_empty' => true,
                      'continue_if_empty' => true, // if true, then continue filtering and validating if input is empty, else input is not filtered/validated
                      ...
                      'validators' => array(array('name' => 'Zend\Validator\NotEmpty',
                                                  'break_chain_on_failure' => true,
                                                  'options' => array('messages' => array(\Zend\Validator\NotEmpty::IS_EMPTY => "shippingStatus cannot be empty"))
                                                 ),
                                            array('name' => 'Zend\I18n\Validator\Alnum',
                                                  'break_chain_on_failure' => true,
                                                  'options' => array('allowWhiteSpace' => false,
                                                                     'messages' => array(\Zend\I18n\Validator\Alnum::INVALID      => "Invalid data type given for shippingStatus",
                                                                                         \Zend\I18n\Validator\Alnum::NOT_ALNUM    => "shippingStatus must only be alphanumeric",
                                                                                         \Zend\I18n\Validator\Alnum::STRING_EMPTY => "shippingStatus cannot be empty",
                                                                                        ),
                                                                    ),
                                                 ),
                                           ),
            ...
            $shippingId = $inputFilter->getValue('shippingId');
            $shippingStatus = $inputFilter->getValue('shippingStatus');
            ...
            $order->updateShippingInfo($shippingId, $shippingStatus);
            
            //remember to save order (persist to storage)
            $orderMapper->save($order);
            
            $response = $this->_createSuccessResponse("shipping info updated successfully");
        }
